...
@endsection
@section('contenido')
    <form action="{{ route('posts.store') }}" name="as" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3/4 bg-gray-400 rounded py-4 px-2 mx-auto w-3/4">
            <div>
                ...
                <input type="text" name="titulo" id="tit" required
            ...
            <div class="mt-2">
                ...
                <input type="text" name="resumen" id="res" required
                    class="focus:ring-indigo-500 focus:border-indigo-500 block w-full pl-2 py-2 sm:text-sm border-gray-300 rounded-md"
                    placeholder="Resumen">
                @error('resumen')
                    <p class="text-sm text-red-500 mt-3">{{ $message }}</p>

...
<div class="w-full">
    <div class="max-w-5xl mx-auto px-6 sm:px-6 lg:px-8 mb-12">
        <div class="bg-gray-900 w-full shadow rounded p-8">
            <form action="{{route('articles.update', $article)}}" name="as" method="post">
                @csrf
                @method("PUT")
                <div class="md:flex items-center mt-12">
                    <div class="w-full md:w-full flex flex-col">
                        <label class="font-semibold leading-none text-gray-300">Nombre</label>
                        <input type="text" name="titulo" value="{{$article->titulo}}" required placeholder="Titulo del Articulo" class="leading-none text-gray-50 p-3 focus:outline-none focus:border-blue-700 mt-4 border-0 bg-gray-800 rounded" />
                        @error('titulo')
                ...
                <div>
                    <div class="w-full flex flex-col mt-8">
                        ...
                        <textarea type="text" name="descripcion" required placeholder="Descripcion del Articulo" class="h-40 text-base leading-none text-gray-50 p-3 focus:outline-none focus:border-blue-700 mt-4 bg-gray-800 border-0 rounded">{{$article->descripcion}}</textarea>
                        @error('descripcion')
                        <p class="text-sm text-red-500 mt-3">{{$message}}</p>
                        @enderror
                    </div>
